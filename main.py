...
X = df.drop(columns=['target', 'speed', 'latitude']).values
y = df['target'].values
...
ax.scatter(
        preprocessing.normalize(X[:, 0].reshape(1, -1), norm="max"),
        preprocessing.normalize(X[:, 1].reshape(1, -1), norm="max"),
        preprocessing.normalize(X[:, 2].reshape(1, -1), norm="max"),
        c=[2-colors_map[e] for e in list(y)]
)
plt.show()
exit()
